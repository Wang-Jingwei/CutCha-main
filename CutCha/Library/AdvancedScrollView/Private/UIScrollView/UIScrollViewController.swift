...
final class UIScrollViewController: UIViewController, UIScrollViewDelegate, UIGestureRecognizerDelegate {
    ...
    @objc func handlePan(gestureRecognizer: UIPanGestureRecognizer) {
        ...
        let location = gestureRecognizer.location(in: contentViewController.view)
        let translation = gestureRecognizer.translation(in: contentViewController.view)
    ...
    // MARK: - UILongPressGestureRecognizerDelegate
    
    typealias LongPressGestureAction = (_ location: CGPoint) -> Void

    func onLongPressGesture(count: Int = 1, perform action: LongPressGestureAction?) {
        if let action = action {
            setupLongPressGesture(count: count, perform: action)
        } else {
    ...
    @objc func handleLongPress(gestureRecognizer: UILongPressGestureRecognizer) {
        guard let longPressGestureAction = longPressGestureAction else {
            return
        }
        if gestureRecognizer.state == UIGestureRecognizer.State.ended {
            let location = gestureRecognizer.location(in: contentViewController.view)
            longPressGestureAction(location)
        }
    }

    private func setupLongPressGesture(count: Int = 1, perform action: @escaping LongPressGestureAction) {
        let selector = #selector(handleLongPress(gestureRecognizer:))
        let gestureRecognizer = UILongPressGestureRecognizer(target: self, action: selector)
        //gestureRecognizer.numberOfTapsRequired = count
        //gestureRecognizer.numberOfTouchesRequired = 1
    ...
    private func resetLongPressGesture() {
        if let longPressGestureRecognizer = longPressGestureRecognizer {
            scrollView.removeGestureRecognizer(longPressGestureRecognizer)
        }

        longPressGestureRecognizer = nil
        longPressGestureAction = nil
    }

    private weak var longPressGestureRecognizer: UILongPressGestureRecognizer?

    private var longPressGestureAction: LongPressGestureAction?
