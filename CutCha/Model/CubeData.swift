...
struct CICubeColorGenerator {
    ...
    func filter() -> CIFilter? {
        ...
        let _ = print("length = \(length)")
        let original = pixels
        
        let imageWidth = self.image.size.width * self.image.scale
        let imageHeight = self.image.size.height * self.image.scale
        
        let _  = print("imageWidth = \(imageWidth) \(imageHeight)")
        let row = Int(imageHeight) / dimension
        let column = Int(imageWidth) / dimension
        
        let _ = print("row = \(row) \(column)")
        
        let _ = print("dimension = \(dimension)")
        // create cube
        ...
        let _ = print("ff* \(origCube[12400]) \(origCube[12500]) \(origCube[12600]) \(origCube[12700])")
