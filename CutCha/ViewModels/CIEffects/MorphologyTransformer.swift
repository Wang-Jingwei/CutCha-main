...
class MorphologyTransformer: ObservableObject, FilterModel {
    
    var photoManager : PhotoManager
    
    init(photoManager : PhotoManager) {
        self.photoManager = photoManager
        self.photoManager.morphologyTransformer = self
    }
    ...
    @Published var diaphragmBladeCount = 8.0 {
        didSet {
    ...
    @Published var angle = Angle(degrees: 45) {
        didSet {
            Task(priority: .userInitiated) {
                await applyBokeh()
            }
    ...
    var sourceRedBuffer: vImage.PixelBuffer<vImage.Planar8>!
    var sourceGreenBuffer: vImage.PixelBuffer<vImage.Planar8>!
    var sourceBlueBuffer: vImage.PixelBuffer<vImage.Planar8>!
    
    /// The destination pixel buffers.
    var destinationRGBBuffer: vImage.PixelBuffer<vImage.Interleaved8x3>!
    var destinationRedBuffer: vImage.PixelBuffer<vImage.Planar8>!
    var destinationGreenBuffer: vImage.PixelBuffer<vImage.Planar8>!
    var destinationBlueBuffer: vImage.PixelBuffer<vImage.Planar8>!
    
    /// Creates a new morphology transformer instance from the supplied image.
    func proceed(sourceImage: UIImage) {
