...
//  SegmentAnywhere
//
//  Created by hansoong choong on 30/5/24.
//

import SwiftUI

struct GaussianView: View {
    ...
    var body: some View {
        ...
        let maxLength = min(currentImage.size.width, currentImage.size.height) / 8
...
//        .border(Color.checkgreen.opacity(0.5))
    ...
    func valueChanged() {
        let effect: CIFilterEffect = .gaussianBlur(radius: radius)
