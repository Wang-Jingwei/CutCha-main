...
public class AngleRuler: UIControl {
    ...
    lazy var valueLabel: UILabel = {
        // vh : adjust height, font
        let label = UILabel(frame: CGRect(origin: .zero, size: CGSize(width: 200, height: 10)))
    ...
    private lazy var midScaleLine: UIView = {
        ...
        view.center = CGPoint(x: self.frame.size.width / 2.0 + pixelOffset, y: self.frame.size.height - bottomMargin - 5)
        return view
    }()

    private lazy var midScaleLineBorder: UIView = {
        let view = UIView(frame: CGRect(x: 0, y: 0, width: 1 + 2 * borderWidth, height: 20 + 2 * borderWidth))
    ...
    private lazy var zeroDot: UIView = {
        ...
        let view = UIView(frame: CGRect(x: x, y: frame.size.height - bottomMargin - 25, width: 4, height: 4))
